...
import com.pmh.iotblog.Adapters.ViewPagerAdapter;
...
public class OnBoardActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        // if button text is next we will go to page of viewpager
            if(btnRight.getText().toString().equals("Next")){
                viewPager.setCurrentItem(viewPager.getCurrentItem()+1);
            }
            else {
                //else its finish will start Auth activity
        ...
        // if btn skip clicked then go to page 3
            viewPager.setCurrentItem(viewPager.getCurrentItem()+2);
        });

    }

    // method to create dots from html
    private void addDots (int position){
        dotsLayout.removeAllViews();
        dots = new TextView[3];
        for (int i = 0 ; i < dots.length; i++){
            dots[i] = new TextView(this);
            // this html code create dot
